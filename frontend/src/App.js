 ...
 * ============================================================
 *
 * When a user taps an Android push notification, sw.js sends:
 *
 * ?push_title=...
 * &push_body=...
 *
 * This component reads those values and displays the
 * SAME message inside EduNotes Pro.
 */
const PushMessageHandler = () => {
  ...
  useEffect(() => {
    try {
      ...
      const pushTitle =
        params.get("push_title");

      const pushBody =
        params.get("push_body");

      /*
       * Nothing to do if this was a normal
       * page visit.
       */
      ...
      /*
       * Prevent the same push from being
       * displayed repeatedly if React renders
       * again.
       */
      const messageKey =
        `${title}::${body}`;
      ...
      const previousMessage =
        sessionStorage.getItem(
          "edunotes-last-push-message"
        );

      if (
        previousMessage !== messageKey
      ) {
        toast(title, {
          description: body,
          duration: 10000,
      ...
      /*
       * Remove push_title and push_body from
       * the browser address bar after reading them.
       *
      ...
      console.error(
        "EduNotes Pro push message handler error:",
        error
      );
    }
  }, [
  ...
  return (
    ...
    <div className="App">
      ...
      <BrowserRouter>
        ...
        {/* Handles messages coming from push notification taps */}
        <PushMessageHandler />
