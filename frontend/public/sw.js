...
self.addEventListener("push", (event) => {
  event.waitUntil(
    (async () => {
      ...
      let notificationUrl = data.url || DEFAULT_URL;
      ...
      try {
        ...
        if (parsed.origin === self.location.origin) {
          /*
           * IMPORTANT:
           * Put the exact notification title and message into
           * the URL so the React app can display it after the
           * notification is opened.
           */
          parsed.searchParams.set("push_title", title);
          parsed.searchParams.set("push_body", body);

          notificationUrl =
            `${parsed.pathname}${parsed.search}${parsed.hash}`;
        } else {
          notificationUrl = DEFAULT_URL;
        }
      } catch {
        notificationUrl = DEFAULT_URL;
      }
        ...
        for (const client of clientList) {
          if ("focus" in client) {
            try {
              ...
              // navigation of an existing client.
            }
